...
class Ueditor extends Controller
{
     ...
     * 获取远程图片
     */
    private function get_remote_image()
    {
        ...
        $uploadMaxFileSize = $uploadSetting["image"]['max_file_size'];
        $uploadMaxFileSize = empty($uploadMaxFileSize) ? 2048 : $uploadMaxFileSize;//默认2M
        $allowedExts = explode(',', $uploadSetting["image"]["allowed"]);
    ...
    {
        $config_text = preg_replace("/\/\*[\s\S]+?\*\//", "", file_get_contents(Env::get('roo_path') . "static/plugins/ueditor/config.json"));
        $config = json_decode($config_text, true);
        $upload_setting = Config('upload.');

        $config['imageMaxSize'] = $upload_setting['file_types']['image']['max_file_size'] * 1024;
        $config['imageAllowFiles'] = array_map([$this, 'ueditorExtension'], explode(",", $upload_setting['file_types']['image']['allowed']));
        $config['scrawlMaxSize'] = $upload_setting['file_types']['image']['max_file_size'] * 1024;
//
        $config['catcherMaxSize'] = $upload_setting['file_types']['image']['max_file_size'] * 1024;
        $config['catcherAllowFiles'] = array_map([$this, 'ueditorExtension'], explode(",", $upload_setting['file_types']['image']['allowed']));

        $config['videoMaxSize'] = $upload_setting['file_types']['video']['max_file_size'] * 1024;
        $config['videoAllowFiles'] = array_map([$this, 'ueditorExtension'], explode(",", $upload_setting['file_types']['video']['allowed']));

        $config['fileMaxSize'] = $upload_setting['file_types']['file']['max_file_size'] * 1024;
        $config['fileAllowFiles'] = array_map([$this, 'ueditorExtension'], explode(",", $upload_setting['file_types']['file']['allowed']));
